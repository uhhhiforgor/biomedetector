...
class MainGUI:   
  ...
  def save_settings(self):  
   ...
   # Update OCRSelector instance with new settings  
   if self.ocr_scanner:  
      self.ocr_scanner.webhook_url = self.webhook_url.get()  
      self.ocr_scanner.private_server_link_url = self.private_server_link_url.get()  
      self.ocr_scanner.glitch_ping_role = self.glitch_ping_role.get()  
      self.ocr_scanner.webhook_urls = [url.get() for url in self.webhook_urls]  
  
      # Update payload dictionary with new settings  
      self.ocr_scanner.payload = {  
        "content": "",  
        "embeds": [  
           {  
              "title": "Biome Started: {}".format(self.ocr_scanner.biome_type),  
              "description": self.ocr_scanner.private_server_link_url,  
              "color": 0x00ff00,  
              "timestamp": datetime.utcnow().isoformat() + "Z",  
              "image": {  
                "url": self.ocr_scanner.image_url  
              }  
           }  
        ]  
      }  
  
      # If biome is glitched, update payload with new glitch ping role  
      if self.ocr_scanner.biome_type == 'GLITCHED':  
        self.ocr_scanner.payload["content"] = f"<@&{self.ocr_scanner.glitch_ping_role}>"   
...
class OCRSelector:   
  ...
  def setup_ui(self):   
    self.status_label = tk.Label(   
      self.root,   
      text="Click to select first corner" if self.scanning_area is None else "Area already selected",   
      font=("Arial", 16),   
      bg='white'   
    )   
  ...
  def send_webhook(self, text, screenshot):  
   ...
   try:  
      ...
      embed_description = self.private_server_link_url  
      if biome_type == 'GLITCHED':  
        embed_description = f"GLITCHED SPOTTED!!! {self.private_server_link_url}"  
      ...
      # If biome is glitched, send a separate message pinging the role  
      if biome_type == 'GLITCHED':  
        payload = {  
           "content": f"<@&{self.glitch_ping_role}>"  
        }  
        for webhook_url in [self.webhook_url] + self.webhook_urls:  
           response = requests.post(webhook_url, json=payload)  
           logger.info(f"Glitch ping response: {response.status_code}")  
  ...
  def periodic_scan(self):   
    ...
    while self.is_scanning:   
      try:   
        ...
        logger.error(f"Periodic scan error: {str(e)}", exc_info=True)   
...
def main():   
  ...
  try:   
    ...
    logger.error(f"Main GUI error: {str(e)}", exc_info=True)   
